...
可运行的[任务流程实验](task-demo/README.md)用本地最小依赖展示提交、执行、查询与恢复；
框架替换不改变本单元的主问题，也不构成 PostgreSQL 并发或真实绘图的验证。

本单元后的既定阶段见 [完整任务流程实践](vertical-task-workflow.md)。讲解覆盖、
用户应用与待确认项见 [收束记录](../../docs/learning-observations/20260909-http-unit-closeout.md)。
