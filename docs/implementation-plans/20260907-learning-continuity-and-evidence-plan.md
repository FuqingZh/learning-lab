...
用户明确授权提交、推送与合并，替代初次实施时不交付的范围限制。当前课程
材料和实现按两个语义提交交付，通过 PR 检查后合并；复验及最终远端状态以
对应 PR 和 CI 为准。React 练习通过显式浏览器可执行文件及独立临时配置完成
交互验证，默认主机包装脚本的局限见练习 README；不扩大教学效果结论。
